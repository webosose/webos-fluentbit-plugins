                            ...
                            struct flb_config *config)
{
    ...
    if (pid_child_python == -1) {
        ...
        close(fd[1]);
        //execv("/usr/bin/python3", paramsList);        
...
//                       dprintf(fd[1], "\n");
                  ...
                  while (msgpack_unpack_next(&result, data, bytes, &off) == MSGPACK_UNPACK_SUCCESS) {
                       ...
                       printf("]\n");
#else
                       fprintf(fwrite,"[%zd] %s: [", cnt++, buf);
                       flb_time_pop_from_msgpack(&tmp, &result, &p);
                       fprintf(fwrite,"%"PRIu32".%09lu, ", (uint32_t)tmp.tm.tv_sec, tmp.tm.tv_nsec);

				       msgpack_object_print(fwrite, *p);
                       fprintf(fwrite,"]\n");
#endif
                  }
